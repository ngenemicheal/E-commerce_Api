    ...
    public void Register(TypeAdapterConfig config)
    {
        config.ForType<CartItem, CartItemResponse>()
            .Map(dest => dest.ProductName,
                src => src.Product != null ? src.Product.Name : $"Product-{src.ProductId}")
        ...
        config.ForType<Cart, CartResponse>()
            ...
            .Map(dest => dest.ItemCount,
                src => src.Items != null ? src.Items.Sum(i => i.Quantity) : 0);

        config.ForType<OrderItem, OrderItemResponse>()
            .Map(dest => dest.UnitPriceSnapshot,
                src => src.UnitPriceSnapshot.Adapt<MoneyDto>())
            .Map(dest => dest.LineTotal, src => src.CalculateLineTotal().Adapt<MoneyDto>());
